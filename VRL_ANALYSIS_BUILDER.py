    ...
    with open(LOG_FILE) as fh:
        for raw in fh:
            ...
            if m:
                ...
                delay = {
                    5:  (float(s5),  float(d5)),
                    10: (float(s10), float(d10)),
                    30: (float(s30), float(d30)),
                    60: (float(s60), float(d60)),
                }
                key = (strat, dir_)
                if key in live and abs(live[key]['entry'] - float(base)) < 1.0:
                    live[key]['delay'] = delay
                for tr in trades:
                    if tr['strategy'] == strat and tr['dir'] == dir_ \
                            and abs(tr['entry'] - float(base)) < 1.0:
                        tr['delay'] = delay
                        break
                continue
...
def delay_str(delay):
    if not delay:
        return "no data yet"
    parts = [f"+{d}s={delay[d][0]:.1f}({'+' if delay[d][1]>=0 else ''}{delay[d][1]:.1f})"
             for d in (5, 10, 30, 60)]
...
def write_analysis(trades, relocks, market):
    ...
    a("## 2. Signal Table")
    a()
    a("| # | Strat | Time | Dir | Strike | Entry | ema9h_gap | gap_vwap | BW | RSI | Peak | PnL V1 | PnL V2 | Trail | Flags |")
    a("|---|-------|------|-----|--------|-------|-----------|----------|----|-----|------|--------|--------|-------|-------|")
    ...
    for tr in sorted(trades, key=lambda x: x['num']):
        ...
        a(f"| S{tr['num']} | {tr['strategy']} | {tr['time']} | {tr['dir']} | {tr['strike']} "
          ...
          f"| {bw} | {tr['rsi']:.1f} | {pk} | {p1} | {p2} | {tl} | {tr['flags']} |")
    a()
    ...
    a(f"| gap_vwap > 8 (overextended) | 0 | {len(far_l)} | Already extended = avoid |")
    ...
    a(f"| FUT_VWAP_MISMATCH | 0 | {len(fut_l)} | Direction conflict = loss |")
    a()
...
if __name__ == '__main__':
    ...
    n, pnl1, pnl2 = write_analysis(trades, relocks, market)
