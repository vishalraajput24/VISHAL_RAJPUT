...
def write_analysis(trades, relocks, market, levels=None, rsi_blocks=None, cross_trades=None, vwap_timeline=None, bwscan_timeline=None):
    ...
    # 8. Reference Trades — benchmarks for ideal setups
    a("---")
    a("## 8. Reference Trades (Benchmarks)")
    a()

    # Reference 1: S11 P2 CE — ideal P2 setup (DTE=1)
    a("### REF-1: S11 P2 CE 23950 | 14:30 | 2026-05-25 | DTE=1 | +36 pts 🔥")
    a()
    a("> Ideal P2 setup — CE below option VWAP, XLEG sharpest of day, spot surge +23 pts.")
    a()
    a("```")
    a("Entry  : 105.5   Exit : 141.5   PnL V1 : +36   PnL V2 : +16   Peak : +43.2")
    a("Trail  : LOCK+36 (LOCK+4 → LOCK+10 → LOCK+12 → LOCK+20 → LOCK+30 → LOCK+36)")
    ...
    a("FUT_VWAP  = +19.8  ✅ spot surged +23 pts in 15 min — real directional move")
    ...
    a("```")
    a()
    a("**Why it worked**: P2 entered below option VWAP (cheap, room to run). Spot broke out +23 pts with")
    a("FUT_VWAP strongly aligned (+19.8). XLEG was the sharpest of the day (-10.5). All 3 pillars aligned.")
    a()

    # Reference 2: S10 P1 PE — ideal P1 setup (DTE=0, late session gamma)
    a("### REF-2: S10 P1 PE 24050 | 12:37 | 2026-05-26 | DTE=0 | +36 pts 🔥")
    a()
    a("> Ideal P1 DTE=0 late-session setup — FUT_VWAP deep BEAR, dual TF HIGH, XLEG all5 clean, gamma explosion.")
    a()
    a("```")
    a("Entry  : 54.4   Exit : 90.4   PnL V1 : +36   PnL V2 : +15   Peak : +41.4")
    a("Trail  : LOCK+36 (LOCK+4 → LOCK+10 → LOCK+12 → LOCK+20 → LOCK+36)")
    a("ema9h_gap = +4.72  ✅ strong above band")
    a("gap_vwap  = +11.09 ⚠️ above option VWAP (P1 entry) — but FUT_VWAP compensated")
    a("FUT_VWAP  = -28.8  ✅ futures -28 pts below VWAP — deep BEAR bias, deepening")
    a("RSI       = 62.1 ↑ ✅ rising, healthy range")
    a("BW        = 6.3    ⚠️ narrow (DTE=0) — but gamma explosion amplified every tick")
    a("XLEG      = CE all5 below EMA9H ✅ — cleanest divergence of the day")
    a("SPOT_EMA  = BEAR ✅ — spot EMA9(24027) < EMA21(24037), trend confirmed")
    a("Dual TF   = HIGH ✅ — PE 1m ABOVE +4.7 || 3m ABOVE +4.0 — both timeframes agreed")
    a("Time      = 12:37  ✅ 53 min to close — DTE=0 gamma peak zone")
    a("DELAY     = SPIKE ❌ — misleading: spot had 5pt dead-cat bounce before real drop")
    a("```")
    a()
    a("**Why it worked — 7 reasons**:")
    a("1. **FUT_VWAP deep BEAR (-28.8, deepening)** — strongest directional signal of the day")
    a("2. **Dual TF HIGH** — PE ABOVE on both 1m AND 3m = confirmed sustained breakout, not spike")
    a("3. **XLEG all5 clean** — CE dead for 5 consecutive candles, zero ambiguity")
    a("4. **SPOT_EMA_BEAR** — spot in downtrend, PE playing WITH the trend")
    a("5. **DTE=0 gamma explosion** — 53 mins to close, options moving 1:1 with spot, +41 pts in 14 min")
    a("6. **XLEG patience** — bot rejected 15 signals at 12:36 waiting for CE all5 to clear, then fired clean")
    a("7. **DELAY was misleading** — SPIKE label was a dead-cat spot bounce, FUT_VWAP told the real story")
    a()
    a("**Key learning**: On DTE=0 late session, trust FUT_VWAP + XLEG all5 + Dual TF HIGH over DELAY label.")
    a("DELAY captures the first 60s — gamma moves happen in minutes 2-15, after DELAY window closes.")
    a()

    # Reference 3: S13 P1 PE — DTE=0 post-market gamma dump
    a("### REF-3: S13 P1 PE 24000 | 13:40 | 2026-05-26 | DTE=0 post-expiry | +36 pts 🔥")
    a()
    a("> Third +36 trade. Post-market-hours DTE=0 gamma dump — FUT_VWAP -49 → -69 → -85 accelerating.")
    a()
    a("```")
    a("Entry  : 47.95   Exit : 84.0   PnL V1 : +36   PnL V2 : +15   Peak : +40.5")
    a("Trail  : LOCK+36 (LOCK+4 → LOCK+10 → LOCK+12 → LOCK+20 → LOCK+30 → LOCK+36)")
    a("ema9h_gap = +4.85  ✅ strong above band")
    a("gap_vwap  = +17.71 ⚠️ slightly above FVS threshold of 15 — just missed FVS filter")
    a("FUT_VWAP  = -49.3 → -69.6 → -85.0 ✅ BEAR accelerating every 15 min — strongest of day")
    a("RSI       = 59.4 ↑ ✅ healthy range, rising")
    a("BW        = 4.2    ⚠️ narrow (DTE=0) — gamma amplified every tick")
    a("XLEG      = CE all5 below EMA9H ✅ — cleanest divergence")
    a("SPOT_EMA  = BEAR ✅ — ema9=23981 < ema21=23990")
    a("DELAY     = MIXED (opt -0.2 at +5s, then +0.4/+4.2/+2.3) — real move confirmed after +30s")
    a("Time      = 13:40 ⚠️ post-market-hours (market closed 13:30) — post-expiry unwinding")
    a("```")
    a()
    a("**Why it worked — same 3 pillars as S10/S11 but stronger:**")
    a("1. **FUT_VWAP -85.0 by 14:00** — deepest BEAR of entire day, accelerating every reading")
    a("2. **Post-expiry gamma unwinding** — after 13:30 close, option sellers hedges unwind, PE rockets")
    a("3. **XLEG all5 clean** — CE completely dead, zero CE momentum")
    a("4. **SPOT_EMA_BEAR** — trend confirmed, PE with the flow")
    a("5. **Pattern match**: identical structure to S10 — only difference was gap_vwap 17.7 vs 11.1")
    a()
    a("**FVS note**: Missed by 2.7 pts (gap_vwap 17.71 vs threshold 15). Next week: calibrate Rule 2 to <18 for DTE=0.")
    a()
    a("**3 Reference trades pattern — what they share:**")
    a("| | REF-1 S11 | REF-2 S10 | REF-3 S13 |")
    a("|--|-----------|-----------|-----------|")
    a("| PnL | +36 | +36 | +36 |")
    a("| XLEG | all5 ✅ | all5 ✅ | all5 ✅ |")
    a("| SPOT_EMA | BULL→PE ✅ | BEAR→PE ✅ | BEAR→PE ✅ |")
    a("| FUT_VWAP | +19.8 BULL ✅ | -28.8 BEAR ✅ | -69.6 BEAR ✅ |")
    a("| Trail hit | LOCK+36 | LOCK+36 | LOCK+36 |")
    a("| Common factor | **FUT_VWAP strongly aligned + XLEG all5** — every single time |")
    a()
