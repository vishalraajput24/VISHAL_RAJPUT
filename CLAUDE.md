...
- **Market-data source is config-driven** (migration in progress, 2026-06-19): `config.yaml` → `data_provider: kite | upstox` → `D.DATA_PROVIDER`. Default **kite**. When `upstox`, the Kite *read* calls (`get_historical_data`, `get_spot_ltp`/`get_vix` REST fallback, `get_lot_size`, `get_nearest_expiry`, `get_option_tokens`) route through `upstox_data.py`. ⚠️ **REST-only — the live tick WebSocket is still Kite.** Do NOT set `upstox` for live trading until the WS feed is migrated (`get_ltp`/option exits would be blind). Moving to Upstox because Kite data API costs ~₹500/mo; Upstox data is free. Orders stay on m.Stock either way. Parity (06-19): spot/VIX/index identical; option 1-min candles differ ~0.2pt avg — within entry noise.
