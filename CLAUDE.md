...
| `screener/smi_focus9.py` | FOCUSED 9-stock engine (owner 2026-06-17, "1 code for the 9 keeper stocks, monitor 1 week"). One engine, each stock running its OWN tuned V12 gate from the 06-17 per-stock tuning (`v12_one_stock_tune.py` / `v12_batch_tune.py`): per-stock **SMI period (k) · cross bands (±ob) · direction · flow on/off** in the `FOCUS` dict. Stocks + win-rates: MARUTI k21/±45/both/flowON 87% · LT k30/±50/both/flowON 87% · TITAN k21/±50/CE/flowON 83% · INFY k30/±35/PE/flowOFF 83% · BHARTIARTL k40/±40/both/flowON 83% · RELIANCE k30/±50/both/flowON 79% · BAJFINANCE k30/±50/both/flowON 79% · TCS k21/±35/both/flowOFF 73% · SBIN k30/±45/both/flowON 71%. Only these 9 ever fire; everything else returns None. Shared exits/fill (1% SL · trail +1.5% · 15:15 EOD · 1 lot ATM). Reuses `smi_paper` + `smi_paper_flow` + `orion_v2514_backtest`; own state/log/tracker (`smi_focus9_state.json` / `smi_focus9_log.csv` / `fno_tracker_focus.csv`, `structure=SMI_FOCUS`), TG "SMI FOCUS9", own cron, log `~/logs/smi_focus9.log`, green "FOCUS9" dashboard badge. **IN-SAMPLE TUNED — this is the ~1-week forward-validation run, NOT a proven edge; judge the week vs the in-sample win-rates above.** |
