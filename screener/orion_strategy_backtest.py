...
def add_intraday_vol_avg(df: pd.DataFrame) -> pd.DataFrame:
    # Cumulative intraday average volume up to (but not including) each bar
    df["_date"] = df.index.date
    df["_cum_vol"] = df.groupby("_date")["volume"].cumsum() - df["volume"]
    df["_bar_num"] = df.groupby("_date").cumcount()  # 0-based bar number within day
    df["vol_intraday_avg"] = df["_cum_vol"] / df["_bar_num"].replace(0, np.nan)
    df.drop(columns=["_date", "_cum_vol", "_bar_num"], inplace=True)
    return df
...
def _build_daily_cross_counts(df: pd.DataFrame) -> dict:
    """
    Returns {(date, bar_index): crosses_so_far_today_before_this_bar}.
    A cross is counted when prev_close and curr_close are on opposite sides of VWAP.
    """
    counts: dict[tuple, int] = {}
    daily_cross: dict = {}   # date -> count so far
    for i in range(1, len(df)):
        ts   = df.index[i]
        d    = ts.date()
        prev = df.iloc[i - 1]
        row  = df.iloc[i]
        if pd.isna(row["vwap"]) or pd.isna(prev["vwap"]):
            counts[(d, i)] = daily_cross.get(d, 0)
            continue
        counts[(d, i)] = daily_cross.get(d, 0)
        prev_side = 1 if float(prev["close"]) >= float(prev["vwap"]) else -1
        curr_side = 1 if float(row["close"])  >= float(row["vwap"])  else -1
        if prev_side != curr_side:
            daily_cross[d] = daily_cross.get(d, 0) + 1
    return counts


def scan_e1(df: pd.DataFrame, vol_filter: str = "none", cross_limit: int = 0) -> list[dict]:
    """
    vol_filter  : "none" | "rolling_1_1" | "intraday_1_3"
    cross_limit : 0 = disabled; N = skip if daily VWAP crosses before this bar >= N
    """
    cross_counts = _build_daily_cross_counts(df) if cross_limit > 0 else {}
    ...
    for i in range(1, len(df)):
        ...
        # Volume filter
        if vol_filter == "rolling_1_1":
            vol_ma = row.get("vol_ma20", np.nan)
            if pd.isna(vol_ma) or float(row["volume"]) < 1.1 * float(vol_ma):
                continue
        elif vol_filter == "intraday_1_3":
            intra_avg = row.get("vol_intraday_avg", np.nan)
            if pd.isna(intra_avg) or float(row["volume"]) < 1.3 * float(intra_avg):
                continue

        # Daily VWAP cross count filter
        if cross_limit > 0:
            crosses = cross_counts.get((ts.date(), i), 0)
...
def _e1_stats(signals: list[dict]) -> dict:
    if not signals:
        return {"n": 0, "win_pct": 0.0, "expectancy": 0.0, "sl_rate": 0.0}
    df = pd.DataFrame(signals)
    n    = len(df)
    wins = (df["result"] == "WIN").sum()
    ...
    return {
        ...
        "expectancy": round(df["pnl_pts"].mean(), 2),
        "sl_rate":    round(sl / n * 100, 1),
    }


def main():
    print("=" * 55)
    print("  ORION Strategy Backtest — Volume Filter Comparison")
    ...
    for sym in ORION_STOCKS:
        ...
        stock_dfs[sym] = prepare(df)
        print(f"    Candles: {len(stock_dfs[sym])}  |  Days: {stock_dfs[sym].index.normalize().nunique()}")

    if not stock_dfs:
        print("\nNo data fetched."); return

    # (label, vol_filter, cross_limit)
    VARIANTS = [
        ("Baseline (no filter)",          "none",        0),
        ("A: rolling 1.1×",               "rolling_1_1", 0),
        ("B: intraday 1.3×",              "intraday_1_3",0),
        ("A+2: rolling 1.1× + cross≤2",   "rolling_1_1", 2),
    ]

    results: dict[str, dict] = {}

    print("\n")
    for label, vf, cl in VARIANTS:
        e1_all = []
        for sym, df in stock_dfs.items():
            e1 = scan_e1(df, vol_filter=vf, cross_limit=cl)
            e1_all.extend(e1)
        results[label] = _e1_stats(e1_all)

    # Comparison table
    print("=" * 70)
    print(f"  {'Filter':<33} {'Signals':>7} {'Win%':>6} {'Expect':>8} {'SL%':>6}")
    print("  " + "-" * 66)
    for label, vf, cl in VARIANTS:
        s = results[label]
        print(f"  {label:<33} {s['n']:>7} {s['win_pct']:>5.1f}% {s['expectancy']:>+7.2f} {s['sl_rate']:>5.1f}%")
    print("=" * 70)

    # Save best-performing variant (highest expectancy) to CSV + report
    best_label = max(results, key=lambda k: results[k]["expectancy"])
    best_entry = next((v, cl) for lbl, v, cl in VARIANTS if lbl == best_label)
    best_vf, best_cl = best_entry
    print(f"\n  Best variant: {best_label}")

    all_signals = []
    for sym, df in stock_dfs.items():
        e1 = scan_e1(df, vol_filter=best_vf, cross_limit=best_cl)
        e2 = scan_e2(df)
        e4 = scan_e4(e2, df)
    ...
    with open(REPORT_FILE, "w") as f:
        f.write(report)
    print(f"  Saved → {OUT_FILE}  ({REPORT_FILE})")
